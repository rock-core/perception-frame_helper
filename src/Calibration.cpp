...
CameraCalibration::CameraCalibration()
    : fx(base::unset<double>())
    , fy(base::unset<double>())
    , cx(base::unset<double>())
    , cy(base::unset<double>())
    , d0(base::unset<double>())
    , d1(base::unset<double>())
    , d2(base::unset<double>())
    , d3(base::unset<double>())
    , width(-1)
    , height(-1)
    , ex(base::unset<double>())
    , ey(base::unset<double>())
    , fisheye(false)
{
}
...
{
    return width > 0 && height > 0 && !base::isUnset<double>(fx) &&
           !base::isUnset<double>(fy) && !base::isUnset<double>(cx) &&
           !base::isUnset<double>(cy) && !base::isUnset<double>(d0) &&
           !base::isUnset<double>(d1) && !base::isUnset<double>(d2) &&
...
ExtrinsicCalibration::ExtrinsicCalibration()
    : tx(base::unset<double>())
    , ty(base::unset<double>())
    , tz(base::unset<double>())
    , rx(base::unset<double>())
    , ry(base::unset<double>())
    , rz(base::unset<double>())
{
}

ExtrinsicCalibration::ExtrinsicCalibration(double tx,
    double ty,
    double tz,
    double rx,
    double ry,
...
{
    return !base::isUnset<double>(tx) && !base::isUnset<double>(ty) &&
           !base::isUnset<double>(tz) && !base::isUnset<double>(rx) &&
           !base::isUnset<double>(ry) && !base::isUnset<double>(rz);
}
    ...
    int height)
{
    ...
    StereoCalibration result = StereoCalibration(CameraCalibration(raw["fc_left"][0],
                                                     raw["fc_left"][1],
                                                     raw["cc_left"][0],
                                                     raw["cc_left"][1],
                                                     raw["kc_left"][0],
                                                     ...
                                                     width,
                                                     height),
        CameraCalibration(raw["fc_right"][0],
            raw["fc_right"][1],
            raw["cc_right"][0],
            raw["cc_right"][1],
            raw["kc_right"][0],
...
{
    return camLeft.isValid() && camRight.isValid() && extrinsic.isValid();
}
