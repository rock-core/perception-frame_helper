...
namespace frame_helper {
    ...
    static const float MAX_H =
        10.0f; // this value is used to convert a 64Bit grayscale image
    ...
    public:
        ...
        // int copyFrameToQImageRGB888(QImage &dst,base::samples::frame::frame_mode_t
        // mode,int pixel_size, int width,int height,const char* pbuffer)
        //{
        int copyFrameToQImageRGB888(QImage& dst,
            ...
            const size_t buffer_size = 0)
        {
            ...
            if ((unsigned int)dst.width() != width ||
                (unsigned int)dst.height() != height ||
                dst.format() != QImage::Format_RGB888) {
            ...
            switch (mode) {
                case base::samples::frame::MODE_UNDEFINED:
                    throw std::runtime_error(
                        " FrameQImageConverter::copyFrameToQImageRGB888: Unknown frame "
                        "mode!");
                    break;
                ...
                case base::samples::frame::MODE_BAYER_GBRG:
                    FrameHelper::convertBayerToRGB24((const uint8_t*)pbuffer,
                        (uint8_t*)dst.bits(),
                        width,
                        height,
                        mode);
                ...
                case base::samples::frame::MODE_BGR:
                    ...
                    // buffer alignment
                    dst = QImage((const uchar*)pbuffer,
                        width,
                        height,
                        width * pixel_size,
                ...
                case base::samples::frame::MODE_PJPG: {
                    FrameHelper::convertPJPGToRGB24((const uint8_t*)pbuffer,
                        (uint8_t*)dst.bits(),
                        buffer_size,
                        width,
                        height);
                ...
                case base::samples::frame::MODE_JPEG: {
                    FrameHelper::convertJPEGToRGB24((const uint8_t*)pbuffer,
                        (uint8_t*)dst.bits(),
                        buffer_size,
                        width,
                        height);
                ...
                case base::samples::frame::MODE_UYVY: {
                    ...
                    for (i = 0; i < height; ++i) {
                        ...
                        for (j = 0; j < width; j += 2) {
                            ...
                            r1 = (255 / 219) * (y1 - 16) +
                                 (255 / 112) * 0.701 * (cr - 128);
                            g1 = (255 / 219) * (y1 - 16) +
                                 (255 / 112) * 0.886 * (0.114 / 0.587) * (cb - 128) -
                                 (255 / 112) * 0.701 * (0.299 / 0.587) * (cr - 128);
                            b1 = (255 / 219) * (y1 - 16) +
                                 (255 / 112) * 0.886 * (cb - 128);
                ...
                case base::samples::frame::MODE_GRAYSCALE: {
                    switch (pixel_size) {
                        ...
                        case 2: {
                            ...
                            cv::Vec3b black =
                                cv::Vec3b(0, 0, 255); //*mat_dst.begin<cv::Vec3b>();
                            for (cv::Mat3b::iterator it = mat_dst.begin<cv::Vec3b>();
                                 it != mat_dst.end<cv::Vec3b>();
                                 ++it) {
                                if (*it == black) {
                                    *it = cv::Vec3b(0, 0, 0);
                                }
                            }
                            dst = QImage((const uchar*)mat_dst.data,
                                width,
                                height,
                                width * 3,
                                QImage::Format_RGB888)
                        ...
                        case 8: // we have to scale the 64 data depth --> a rgb color
                                // coding is used
                        {
                            static const float SCALE_H = 1 / MAX_H;
                            static const double SCALE_S =
                                1 / floor(std::numeric_limits<double>::max() * SCALE_H);
                ...
                case base::samples::frame::MODE_PNG: {
                    char* buffer = const_cast<char*>(
                        pbuffer); // workaround for Mat non const constructor
                    const cv::Mat input(1,
                        buffer_size,
                        CV_8UC1,
                        buffer); // ensure buffer will not be modified !!!
                    cv::Mat img = cv::imdecode(input, cv::IMREAD_COLOR);
                    dst = QImage((const uchar*)img.data,
                        img.cols,
                        img.rows,
                        QImage::Format_RGB888);
                    dst = dst.rgbSwapped();
                ...
                default:
                    throw std::runtime_error(
                        " FrameQImageConverter::copyFrameToQImageRGB888: Can not convert "
                        "frame to RGB888!");
            }
            return ireturn;
        };
        ...
        int copyFrameToQImageRGB888(QImage& dst,
            ...
            const size_t size = 0)
        {
            base::samples::frame::frame_mode_t _mode =
                base::samples::frame::Frame::toFrameMode(mode.toStdString());
            return copyFrameToQImageRGB888(dst,
                _mode,
                pixel_size,
                width,
