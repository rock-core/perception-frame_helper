...
LinearExposureController::LinearExposureController(int targetBrightness, int factor, int tolerance) {
...
int LinearExposureController::getNewExposure(int currentBrightness) {
	if((currentBrightness < (targetBrightness - tolerance)) ||
			(currentBrightness > (targetBrightness + tolerance))) {
