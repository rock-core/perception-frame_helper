...
namespace frame_helper {
    ...
    void ColorGradient::addColorPoint(const float red,
        const float green,
        const float blue,
        const float value)
    {
        for (std::list<ColorPoint>::iterator it = color.begin(); it != color.end();
             ++it) {
            if (value < it->val) {
    ...
    void ColorGradient::getColorAtValue(const float value,
        float& red,
        float& green,
        float& blue) const
    {
        ...
        int index = 0;
        for (std::list<ColorPoint>::const_iterator it = color.begin(); it != color.end();
             ++it, ++index) {
            ColorPoint currC = *it;
            if (value < currC.val) {
        ...
        switch (type) {
            ...
            default:
                throw std::invalid_argument(
                    "Color gradient type does not match a known value");
                break;
        }
    }
