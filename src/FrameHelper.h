...
namespace frame_helper {
    ...
    public:
        ...
        // this is none static because an internal buffer is used which is not be resized
        // if the src and dst have always the same attributes
        void convert(const base::samples::frame::Frame& src,
            ...
            UndistortAlgorithm const& undistort_algorithm = UNDISTORT_CUBIC);
        ...
        void undistort(const base::samples::frame::Frame& src,
            base::samples::frame::Frame& dst,
            UndistortAlgorithm const& resize_algorithm);

        // copies a cv::Mat into a Frame
        // frame is initialized if it has not got the right size
        static void copyMatToFrame(const cv::Mat& src,
            base::samples::frame::Frame& frame);

        // resizes a bayer image without converting it to another color format by skipping
        // pixels The size of src must be a multiple of two times dst size otherwise an
        // exception will be thrown
        static void resize_bayer(const base::samples::frame::Frame& src,
            base::samples::frame::Frame& dst,
            int offset_x = 0,
            int offset_y = 0);
        ...
        static void resize(const base::samples::frame::Frame& src,
            base::samples::frame::Frame& dst,
            int offset_x = 0,
            int offset_y = 0,
        ...
        // parameters:
        // f                 focal length of the camera in pixels which has taken the
        // image
        ...
        // calculates the distance in meters to an object seen on an image
        // parameters:
        // frame             frame which must have the additional attributes fx (focal
        // length) and fy virtual_width     width of the object in pixels real_width width
        // of the object in meters virtual_height    height of the object in pixels
        ...
        static cv::Point2f calcRelPosToPoint(float fx,
            float fy,
            int x1,
            int y1,
            int x2,
        ...
        static cv::Point2f calcRelPosToPoint(const base::samples::frame::Frame& frame,
            ...
            int y2,
            float d);
        static cv::Point2f calcRelPosToCenter(const base::samples::frame::Frame& frame,
            int x1,
            int y1,
            float d);
        ...
        // converts a bayer pattern image to rgb image which has 8 bit data depth for each
        // channel
        static void convertBayerToRGB24(const base::samples::frame::Frame& src,
            base::samples::frame::Frame& dst);
        static void convertBayerToRGB24(const uint8_t* src,
            uint8_t* dst,
            int width,
            int height,
        ...
        static void convertRGBToGray(const base::samples::frame::Frame& src,
            base::samples::frame::Frame& dst,
        ...
        static void convertBayerToGreenChannel(const base::samples::frame::Frame& src,
            base::samples::frame::Frame& dst);
        static void convertBayerToGreenChannel(const uint8_t* src,
            uint8_t* dst,
            int width,
            int height,
            base::samples::frame::frame_mode_t mode);
        ...
        static void calcDiff(const base::samples::frame::Frame& src1,
         ...
         */
        static void convertPJPGToRGB24(const base::samples::frame::Frame& src,
            base::samples::frame::Frame& dst);
        static bool convertPJPGToRGB24(const uint8_t* source,
        ...
        static void convertJPEGToRGB24(uint8_t const* src,
            uint8_t* dst,
        ...
        /**
         * Loads a JPEG image from file without decompressing it (can be useful if frame
         * is directly transmitted afterwards)
         */
        static void loadFrameJPEG(const std::string& filename,
            base::samples::frame::Frame& frame);
        /**
         * Loads JPEG image from memory without decompressing it.
         * If frame.size=={0,0} before calling this function, it is set automatically
         * based on the JPEG header.
         */
        static void loadFrameJPEG(const uint8_t* src,
            size_t size,
            base::samples::frame::Frame& frame);
    };
};
