...
namespace frame_helper
{
    class CV_EXPORTS_W FrameHelper
    {
	    ...
	    // which uses R and P matrices for stereo calibration as well
            ...
            CV_WRAP static void copyMatToFrame(const cv::Mat &src,CV_OUT base::samples::frame::Frame &frame);
            ...
            CV_WRAP static void resize_bayer(const base::samples::frame::Frame &src,CV_OUT base::samples::frame::Frame &dst,
                    int offset_x = 0, int offset_y = 0);

            //resize a frame
            //the size is implied by the dst frame
            CV_WRAP static void resize(const base::samples::frame::Frame &src,CV_OUT base::samples::frame::Frame &dst,
                    int offset_x = 0, int offset_y = 0,int algo = INTER_LINEAR);

            //rotates a frame by 180 degrees
            //dst is inititialized as a copy of src with rotated image
            CV_WRAP static void rotateBy180Degrees(const base::samples::frame::Frame &src,
                    CV_OUT base::samples::frame::Frame &dst);
            ...
            CV_WRAP static float calcDistanceToObject(float f,float virtual_size,float real_size);
            ...
            CV_WRAP static float calcDistanceToObject(const base::samples::frame::Frame &frame,
            ...
            CV_WRAP static cv::Point2f calcRelPosToPoint(float fx,float fy, int x1,int y1,int x2,int y2, float d);
            CV_WRAP static cv::Point2f calcRelPosToPoint(const base::samples::frame::Frame &frame,
                                             int x1,int y1,int x2,int y2, float d);
            CV_WRAP static cv::Point2f calcRelPosToCenter(const base::samples::frame::Frame &frame,int x1,int y1,float d);

            //converts src to dst 
            //the mode of dst implies the conversion 
            CV_WRAP void convertColor(const base::samples::frame::Frame &src,CV_OUT base::samples::frame::Frame &dst);
            ...
            CV_WRAP static void convertRGBToGray(const base::samples::frame::Frame &src,CV_OUT base::samples::frame::Frame &dst,bool copy_attributes =true);

            //copies only the green channel of a bayer pattern image to dst
            //dst has to be MODE_GRAYSCALE
            CV_WRAP static void convertBayerToGreenChannel(const base::samples::frame::Frame &src,CV_OUT base::samples::frame::Frame &dst);
            ...
            CV_WRAP static void calcDiff(const base::samples::frame::Frame &src1,const base::samples::frame::Frame &src2, CV_OUT base::samples::frame::Frame &dst);
			
	    /**
	     * /Brief conversion for PJPG to RGB24.
	    */
	    CV_WRAP static void convertPJPGToRGB24(const base::samples::frame::Frame &src, CV_OUT base::samples::frame::Frame &dst);
	    CV_WRAP static bool convertPJPGToRGB24(const uint8_t *source, uint8_t *target_buffer,const size_t source_size, const int width, const int height);

            // JPEG to RGB24 conversion using JpegConversion.
            CV_WRAP static void convertJPEGToRGB24(uint8_t const* src, uint8_t* dst, size_t const src_size, int const width, int const height);
	    
	    CV_WRAP static cv::Mat convertToCvMat(const base::samples::frame::Frame &frame);
	    
	    CV_WRAP static int getOpenCvType(const base::samples::frame::Frame &frame);
    
            CV_WRAP void saveFrame(const cv::String &filename,base::samples::frame::Frame const &frame);
            CV_WRAP void loadFrame(const cv::String &filename,CV_OUT base::samples::frame::Frame &frame);
    };
};
#endif // FRAMEHELPER_H
