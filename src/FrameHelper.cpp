...
namespace frame_helper {
    ...
    {
        return cv::Mat(frame.size.height,
            frame.size.width,
            getOpenCvType(frame),
            (void*)frame.getImageConstPtr());
    }
    ...
    bool FrameHelper::convertPJPGToRGB24(const uint8_t* source,
        ...
        const int height)
    {
        ...
        // Do Conversion
        // int bytes = v4lconvert_convert(v4lconvert_data,&src_fmt,&fmt,(unsigned
        // char*)source,source_size,(unsigned char*)target_buffer,fmt.fmt.pix.sizeimage);
        int bytes = v4lconvert_convert(v4lconvert_data,
            &src_fmt,
            &fmt,
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                target_buffer[y * (width * 3) + (x * 3) + 0] =
                    tmp[x * (height * 3) + (y * 3) + 0];
                target_buffer[y * (width * 3) + (x * 3) + 1] =
                    tmp[x * (height * 3) + (y * 3) + 1];
                target_buffer[y * (width * 3) + (x * 3) + 2] =
                    tmp[x * (height * 3) + (y * 3) + 2];
            }
        }
#else
        ...
        ResizeAlgorithm const& undistort_algorithm)
    {
        ...
        if (src.getFrameMode() != dst.getFrameMode() ||
            src.getDataDepth() != dst.getDataDepth()) {
            mode += COLOR;
        }
        ...
        ResizeAlgorithm const& undistort_algorithm)
    {
        ...
        if (src.getFrameMode() != MODE_RGB && src.getFrameMode() != MODE_GRAYSCALE)
            throw std::runtime_error(
                "FrameHelper::undistort: frame mode is not supported!");
        ...
        if (!calibration.getCalibration().isValid()) {
            ...
            if (!calibration.getCalibration().isValid())
                throw std::runtime_error(
                    "Could not get valid calibration parameters for undistort.");
        }
        ...
        ResizeAlgorithm algo)
    {
        ...
        if (src.getFrameMode() != dst.getFrameMode())
            throw std::runtime_error("FrameHelper::resize: Cannot resize frame. Dst and "
                                     "src have different frame modes.");

        // when compressed the image should not be resized
        if (src.isCompressed()) {
            throw std::runtime_error(
                "FrameHelper::resize: Compressed frames cannot be resized\
 yet, set 'width' and 'height' in the startup-script accordingly to the delivered camera image");
        ...
        if (offset_x != 0 || offset_y != 0)
            cv_src =
                cv::Mat(cv_src, cv::Rect(offset_x, offset_y, cv_dst.cols, cv_dst.rows));

        switch (algo) {
            case INTER_LINEAR:
                cv::resize(cv_src,
                    cv_dst,
                    cv::Size(cv_dst.cols, cv_dst.rows),
                    0,
                    0,
                    cv::INTER_LINEAR);
                break;
    ...
    void FrameHelper::resize_bayer(const base::samples::frame::Frame& src,
        base::samples::frame::Frame& dst,
        int offset_x,
        int offset_y)
    {
        ...
        if (!src.isBayer())
            throw std::runtime_error("FrameHelper::resize_bayer: Cannot resize frame. "
                                     "Src mode is not MODE_BAYER.");

        // check if both images have the same color format
        if (src.getFrameMode() != dst.getFrameMode())
            throw std::runtime_error("FrameHelper::resize_bayer: Cannot resize frame. "
                                     "Dst and src have different frame modes.");
        ...
        if (src_width % dst.size.width || src_height % dst.size.height ||
            resize_factor_x != 2 || resize_factor_y != 2)
            throw std::runtime_error("FrameHelper::resize_bayer: cannot resize frame. "
    ...
    void FrameHelper::convertColor(const base::samples::frame::Frame& src,
        base::samples::frame::Frame& dst)
    {
        switch (src.getFrameMode()) {
                ...
                switch (dst.getFrameMode()) {
                    ...
                    case MODE_BGR:
                        if (src.getDataDepth() != dst.getDataDepth())
                            throw std::runtime_error(
                                "FrameHelper::convertColor: Cannot convert frame mode "
                                "bgr to bgr with different data depths. Conversion is "
                                "not implemented.");
                        else
                            dst.init(src, true);
                        break;
                    case MODE_PJPG:
                        throw std::runtime_error(
                            "FrameHelper::convertColor: Cannot convert frame mode bgr to "
                            "pjpg. Conversion is not implemented.");
                        break;
                    ...
                    case MODE_RGB:
                        if (src.getDataDepth() != dst.getDataDepth())
                            throw std::runtime_error(
                                "FrameHelper::convertColor: Cannot convert frame mode "
                                "bgr to rgb with different data depths. Conversion is "
                                "not implemented.");
                        else {
                            const cv::Mat cv_src = FrameHelper::convertToCvMat(src);
                            dst.init(cv_src.cols,
                                cv_src.rows,
                                src.getDataDepth(),
                                dst.frame_mode,
                                -1);
                    ...
                    case MODE_BAYER:
                        throw std::runtime_error(
                            "FrameHelper::convertColor: Cannot convert frame mode bgr to "
                            "bayer pattern. Please specify bayer pattern "
                            "(RGGB,GRBG,BGGR,GBRG).");
                        break;
                    ...
                    case MODE_BAYER_GBRG:
                        throw std::runtime_error(
                            "FrameHelper::convertColor: Cannot convert frame mode bgr to "
                            "bayer pattern. Conversion is not implemented.");
                        break;
                        // BGR --> PNG
                    case MODE_PNG: {
                        const cv::Mat cv_src = FrameHelper::convertToCvMat(src);
                    ...
                    default:
                        throw std::runtime_error("FrameHelper::convertColor: Cannot "
                                                 "convert frame mode - mode is unknown");
                }
                break;
                ...
                switch (dst.getFrameMode()) {
                    ...
                    case MODE_RGB:
                        if (src.getDataDepth() != dst.getDataDepth())
                            throw std::runtime_error(
                                "FrameHelper::convertColor: Cannot convert frame mode "
                                "rgb to rgb with different data depths. Conversion is "
                                "not implemented.");
                        else
                            dst.init(src, true);
                        break;
                    ...
                    case MODE_BGR:
                        if (src.getDataDepth() != dst.getDataDepth())
                            throw std::runtime_error(
                                "FrameHelper::convertColor: Cannot convert frame mode "
                                "rgb to bgr with different data depths. Conversion is "
                                "not implemented.");
                        else {
                            const cv::Mat cv_src = FrameHelper::convertToCvMat(src);
                            dst.init(cv_src.cols,
                    ...
                    case MODE_BAYER:
                        throw std::runtime_error(
                            "FrameHelper::convertColor: Cannot convert frame mode rgb to "
                            "bayer pattern. Please specify bayer pattern "
                            "(RGGB,GRBG,BGGR,GBRG).");
                        break;
                ...
                switch (dst.getFrameMode()) {
                    ...
                    case MODE_GRAYSCALE:
                        if (src.getDataDepth() != dst.getDataDepth())
                            throw std::runtime_error(
                                "FrameHelper::convertColor: Cannot convert frame mode "
                                "grayscale to grayscale with different data depths. "
                    ...
                    default:
                        throw std::runtime_error(
                            "FrameHelper::convertColor: Cannot convert frame mode "
                            "grayscale to ?. Conversion is not implemented.");
                        break;
                }
                break;
            ...
            case MODE_PJPG: {
                switch (dst.getFrameMode()) {
                    ...
                    case MODE_PJPG:
                        ...
                        throw std::runtime_error(
                            "FrameHelper::convertColor: Cannot convert frame mode pjpg "
                            "to gray pattern. Conversion not implements.");
                        break;
                    ...
                    case MODE_BAYER:
                        throw std::runtime_error(
                            "FrameHelper::convertColor: Cannot convert frame mode pjpg "
                            "to bayer pattern. Please specify bayer pattern "
                            "(RGGB,GRBG,BGGR,GBRG).");
                        break;
                    ...
                    case MODE_BAYER_GBRG:
                        throw std::runtime_error(
                            "FrameHelper::convertColor: Cannot convert frame mode pjpg "
                            "to bayer pattern. Conversion is not implemented.");
                        break;
                    ...
                    case MODE_JPEG: {
                        ...
                        conversion::JpegConversion jpeg_conversion;
                        jpeg_conversion.compress(frame_tmp,
                            dst); // Copies image independent attributes as well.
                        break;
                    }
                    default:
                        throw std::runtime_error(
                ...
                case MODE_BAYER:
                    throw std::runtime_error(
                        "FrameHelper::convertColor: Cannot convert frame mode bayer to "
                        "?. Please specify bayer pattern (RGGB,GRBG,BGGR,GBRG).");
                    break;
            }
            ...
            case MODE_BAYER_GBRG:
                switch (dst.getFrameMode()) {
                    ...
                    case MODE_PJPG:
                        throw std::runtime_error(
                            "FrameHelper::convertColor: Cannot convert frame mode bayer "
                            "to pjpg. Conversion is not implemented.");
                        break;

                    case MODE_JPEG: {
                        base::samples::frame::Frame frame_tmp(src.getWidth(),
                            src.getHeight(),
                            src.getDataDepth(),
                            base::samples::frame::MODE_RGB,
                            -1);
                        convertBayerToRGB24(src.getImageConstPtr(),
                            frame_tmp.getImagePtr(),
                            src.getWidth(),
                            src.getHeight(),
                            src.getFrameMode());
                    ...
                    case MODE_BGR:
                        if (src.getDataDepth() != dst.getDataDepth())
                            throw std::runtime_error(
                                "FrameHelper::convertColor: Cannot convert frame mode "
                                "bayer to bgr with different data depths. Conversion is "
                                "not implemented.");
                        dst.init(src.getWidth(),
                            ...
                            MODE_RGB,
                            -1);
                        frame_buffer3.init(src.getWidth(),
                            src.getHeight(),
                            src.getDataDepth(),
                            dst.getFrameMode(),
                            -1);
                        convertBayerToRGB24(src.getImageConstPtr(),
                            frame_buffer3.getImagePtr(),
                            src.getWidth(),
                            src.getHeight(),
                            src.frame_mode);
                        {
                            cv::Mat cv_dst = FrameHelper::convertToCvMat(dst);
                            cv::cvtColor(FrameHelper::convertToCvMat(frame_buffer3),
                                cv_dst,
                                cv::COLOR_RGB2BGR);
                            dst.copyImageIndependantAttributes(src);
                        }
                    ...
                    case MODE_RGB:
                        if (src.getDataDepth() != dst.getDataDepth())
                            throw std::runtime_error(
                                "FrameHelper::convertColor: Cannot convert frame mode "
                                "bayer to rgb with different data depths. Conversion is "
                                "not implemented.");
                        dst.init(src.getWidth(),
                            src.getHeight(),
                            src.getDataDepth(),
                    ...
                    case MODE_GRAYSCALE:
                        if (src.getDataDepth() != dst.getDataDepth())
                            throw std::runtime_error(
                                "FrameHelper::convertColor: Cannot convert frame mode "
                                "bayer to grayscale with different data depths. "
                                "Conversion is not implemented.");
                    ...
                    case MODE_BAYER_GBRG:
                        if (src.getDataDepth() == dst.getDataDepth() &&
                            dst.getFrameMode() == src.getFrameMode())
                            dst.init(src, true);
                        else
                            throw std::runtime_error(
                                "FrameHelper::convertColor: Cannot convert frame mode "
                                "bayer to bayer pattern with different pattern or data "
                                "depth. Conversion is not implemented.");
                        break;
            ...
            case MODE_JPEG: {
                ...
                switch (dst.getFrameMode()) {
                    // JPEG --> RGB
                    case MODE_RGB: {
                        jpeg_conversion.decompress(src,
                            base::samples::frame::MODE_RGB,
                            dst);
                        break;
                    }
                    ...
                    case MODE_BAYER:
                        throw std::runtime_error(
                            "FrameHelper::convertColor: Cannot convert frame mode jpeg "
                            "to bayer pattern. Please specify bayer pattern "
                            "(RGGB,GRBG,BGGR,GBRG).");
                        break;
                    ...
                    case MODE_BAYER_GBRG:
                        throw std::runtime_error(
                            "FrameHelper::convertColor: Cannot convert frame mode jepg "
                            "to bayer pattern. Conversion is not implemented.");
                        break;
            ...
            case MODE_PNG: {
                ...
                switch (dst.getFrameMode()) {
                    case MODE_RGB: {
                        ...
                        cv::cvtColor(out,
                            FrameHelper::convertToCvMat(dst),
                            cv::COLOR_BGR2RGB);
                        break;
                    }
                    ...
                    default:
                        throw std::runtime_error(
                            "FrameHelper::convertColor: Cannot convert frame mode png to "
                            "requested mode. Requested mode is unknown");
                } // end dst.getFrameMode()
            }
            default:
                throw std::runtime_error("FrameHelper::convertColor: Cannot convert "
                                         "frame mode ?- mode is unknown");
        } // end switch(src.getFrameMode())
    }

    // only a data depth of 1 Byte is supported
    void FrameHelper::convertRGBToGray(const base::samples::frame::Frame& src,
        base::samples::frame::Frame& dst,
        bool copy_attributes)
    {
        if (src.getPixelSize() != 3)
            throw std::runtime_error("FrameHelper::convertRGBToGray: Can only convert "
                                     "frame mode rgb 24 bit to grayscale 8 bit!");
        ...
        if (src.getWidth() != dst.getWidth() || src.getHeight() != dst.getHeight() ||
            dst.getFrameMode() != MODE_GRAYSCALE || dst.getPixelSize() != 1) {
    ...
    cv::Point2f FrameHelper::calcRelPosToPoint(float fx,
        float fy,
        int x1,
        int y1,
        int x2,
        ...
        float d)
    {
        if (!frame.hasAttribute("fx"))
            throw std::runtime_error(
                "FrameHelper::calcDistanceToObject: frame has no attribute fx");
        if (!frame.hasAttribute("fy"))
            throw std::runtime_error(
                "FrameHelper::calcDistanceToObject: frame has no attribute fy");
    ...
    cv::Point2f FrameHelper::calcRelPosToCenter(const base::samples::frame::Frame& frame,
        ...
        float d)
    {
        return calcRelPosToPoint(frame,
            x1,
            y1,
            frame.getWidth() * 0.5,
            frame.getHeight() * 0.5,
    ...
    void FrameHelper::convertBayerToRGB24(const base::samples::frame::Frame& src,
        base::samples::frame::Frame& dst)
    {
    ...
    void FrameHelper::convertBayerToRGB24(const uint8_t* src,
        uint8_t* dst,
        int width,
        int height,
        base::samples::frame::frame_mode_t mode)
    {
        ...
        int blue = mode == MODE_BAYER_RGGB || mode == MODE_BAYER_GRBG ? 1 : -1;
        int start_with_green = mode == MODE_BAYER_GBRG || mode == MODE_BAYER_GRBG;
        int i, imax, iinc;

        if (!(mode == MODE_BAYER_RGGB || mode == MODE_BAYER_GBRG ||
                mode == MODE_BAYER_GRBG || mode == MODE_BAYER_BGGR))
            throw std::runtime_error(
                "Helper::convertBayerToRGB24: Unknown Bayer pattern");
    ...
    void FrameHelper::calcDiff(const base::samples::frame::Frame& src1,
        ...
        base::samples::frame::Frame& dst)
    {
        ...
        if (frame_total_size != static_cast<int>(src2.getNumberOfBytes()))
            throw std::runtime_error(
                "calcDiff: size missmatch between src1 and src2 --> can not calc diff! ");
    ...
    void FrameHelper::convertBayerToGreenChannel(const base::samples::frame::Frame& src,
        base::samples::frame::Frame& dst)
    {
        // check dst format
        if (dst.frame_mode != MODE_GRAYSCALE)
            std::runtime_error(
                "FrameHelper::converBayerToGreenChannel: Cannot convert frame mode bayer "
                "to green channel. Dst image must be of mode MODE_GRAYSCALE.");

        if (src.getDataDepth() != dst.getDataDepth())
            std::runtime_error(
                "FrameHelper::converBayerToGreenChannel: Cannot convert frame mode bayer "
                "to green channel. src and dst must have the same data depth.");
        ...
        dst.init(src.getWidth(),
            ...
            dst.getFrameMode(),
            -1);
        convertBayerToGreenChannel(src.getImageConstPtr(),
            dst.getImagePtr(),
            src.getWidth(),
            src.getHeight(),
            src.frame_mode);
    ...
    void FrameHelper::convertBayerToGreenChannel(const uint8_t* src,
        uint8_t* dst,
        int width,
        int height,
        base::samples::frame::frame_mode_t mode)
    ...
    void FrameHelper::convertPJPGToRGB24(const base::samples::frame::Frame& src,
        base::samples::frame::Frame& dst)
    {
        // check src format
        if (src.frame_mode != MODE_PJPG)
            std::runtime_error(
                "FrameHelper::convertPJPGToRGB888: Source frame mode must be MODE_PJPG.");

        // check dst format
        if (dst.frame_mode != MODE_RGB)
            std::runtime_error("FrameHelper::convertPJPGToRGB888: Dst frame mode must be "
                               "of mode MODE_RGB.");

        dst.init(src.getWidth(), src.getHeight(), 8, dst.getFrameMode(), -1);
        convertPJPGToRGB24(src.getImageConstPtr(),
            dst.getImagePtr(),
            src.image.size(),
            src.getWidth(),
            src.getHeight());
    ...
    void FrameHelper::copyMatToFrame(const cv::Mat& src,
        base::samples::frame::Frame& frame)
    {
        ...
        switch (src.type()) {
            ...
            default:
                throw std::runtime_error(
                    "Unknown format. Can not convert cv:Mat to Frame.");
        }
    ...
    void FrameHelper::convertJPEGToRGB24(uint8_t const* src,
        uint8_t* dst,
    ...
    void FrameHelper::saveFrame(const std::string& filename,
        const base::samples::frame::Frame& frame)
    {
        base::samples::frame::Frame temp;
        frame_mode_t mode =
            frame.frame_mode == MODE_GRAYSCALE ? MODE_GRAYSCALE : MODE_BGR;
    ...
    void FrameHelper::loadFrame(const std::string& filename,
        base::samples::frame::Frame& frame)
    {
    ...
    void FrameHelper::loadFrameJPEG(const std::string& filename,
        base::samples::frame::Frame& frame)
    {
        conversion::JpegConversion::loadJpeg(filename, 0, 0, frame);
    }
    void FrameHelper::loadFrameJPEG(const uint8_t* src,
        size_t size,
        base::samples::frame::Frame& frame)
    {
